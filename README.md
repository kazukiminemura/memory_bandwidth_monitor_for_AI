...
A real-time CLI tool that shows system memory state and estimates annotated workload memory traffic.
...
- System mode (default):
  ...
  - Actual system memory capacity and pressure
  - Optional synthetic read/write bandwidth probe with `--probe-bandwidth`
...
Default system memory view:

```powershell
./build/Release/mbm_cli.exe
```

Run and tune the synthetic bandwidth probe:

```powershell
./build/Release/mbm_cli.exe --probe-bandwidth --workers 4 --probe-mb 256 --interval-ms 500
```

Use Intel PCM hardware counters for actual DRAM read/write bandwidth:

```powershell
./build/Release/mbm_cli.exe --counter-backend intel-pcm --pcm-path C:\tools\pcm\pcm-memory.exe
```

`pcm-memory.exe` must be installed separately and may need an elevated terminal
and the Intel PCM Windows driver setup. If `pcm-memory.exe` is on `PATH`, the
`--pcm-path` option can be omitted.
...
- Default system mode does not report CPU DRAM read/write bandwidth because Windows system information does not expose actual DRAM bandwidth counters.
- `--probe-bandwidth` measures read and write bandwidth with a synthetic memory probe; those numbers are generated by this tool and are not idle system traffic.
- `--counter-backend intel-pcm` shells out to Intel PCM `pcm-memory` CSV output and sums socket-level `Mem Read (MB/s)` and `Mem Write (MB/s)` columns.
...
- `--pid` and `--process-name` add process memory state only. Per-process memory bandwidth needs a hardware-counter backend.
...
Window: 675 ms   Workers: 6   BW: off   Max: 51.2 GB/s   Sample: live counters only
 RAM  [||||||||||||||||||                      ]    14.2 GB /    31.7 GB  load 44%
 Free [||||||||||||||||||||||                  ]    17.5 GB available
 Cmit [|||||||||||||||||||                     ]    23.7 GB /    50.9 GB committed
 Read [                                        ]          -- /  51.2 GB/s  actual DRAM read bandwidth unavailable
Write [                                        ]          -- /  51.2 GB/s  actual DRAM write bandwidth unavailable
